...
Query.prototype.rangeFilter = function(options){
  ...
  if(Array.isArray(options)){
    for (let option of options) {
      var parameter = 'fq=';
      parameter += option.field + ':' + '[' + encodeURIComponent(option.start) + '%20TO%20' + encodeURIComponent(option.end) + ']';
...
* @param {Array|Object} options -
 * @param {String} options.field - the name of the field where the range is applied
 * @param {String|Number|Date} options.value - the field value
 *
 * @return {Query}
 * @api public
 *
 * @example
 * var query = client.createQuery();
 * query.q({ '*' : '*' }).matchFilter({ field : 'id', value : 100})
 * query.q({ '*' : '*' }).matchFilter('id', 100)
 * // also works
 * query.q({ '*' : '*' }).matchFilter([{ field : 'id', value : 100},{ field : 'date', value : new Date()}]);
 */

Query.prototype.matchFilter = function (options) {
  var self = this;
  options = format.dateISOify(options);
  if (Array.isArray(options)) {
    for (let option of options) {
      var parameter = 'fq=';
      parameter += option.field + ':' + encodeURIComponent(option.value);
      this.parameters.push(parameter);
    }
  } else {
    var parameter = 'fq=';
 ...
 * @param {String|Array} [options.query] - This parameter allows you to specify an arbitrary query in the Lucene default syntax to generate a facet count. By default, faceting returns a count of the unique terms for a "field", while facet.query allows you to determine counts for arbitrary terms or expressions.
...
Query.prototype.facet = function (options) {
  ...
  if (options.query) {
    options.query = arrayUtils.toArray(options.query);
    options.query.forEach(function (query) {
      self.parameters.push('facet.query=' + encodeURIComponent(options.query))
    });
  }
  if (options.field) {
    options.field = arrayUtils.toArray(options.field);
